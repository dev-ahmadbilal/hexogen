...
---
import { <%= version === 'v1' ? `${entity}${name}` : `${entity}${name}${version.toUpperCase()}` %> } from "<%= version === 'v1' ? `../../domain/queries/${h.inflection.transform(entity, ['underscore', 'dasherize'])}-${h.inflection.transform(name, ['underscore', 'dasherize'])}` : `../../domain/queries/${h.inflection.transform(entity, ['underscore', 'dasherize'])}-${h.inflection.transform(name, ['underscore', 'dasherize'])}.${version}` %>";
